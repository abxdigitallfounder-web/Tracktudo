...
export function CampaignsPage({ reloadKey }: { reloadKey: number }) {
  ...
  const load = useCallback(
    async (showLoading: boolean) => {
      ...
      try {
        const [resp, accs, st] = await Promise.all([
          apiGetCampaigns({ since, until, search: search.trim() || undefined, status: statusFilter || undefined, accountId: accountFilter || undefined }),
          accounts.length > 0 ? Promise.resolve(accounts) : apiGetAccounts(),
    ...
    [since, until, search, statusFilter, accountFilter],
  );
  ...
  const filteredRows = useMemo(
    () => (productFilter ? rows.filter((r) => r.product === productFilter) : rows),
    [rows, productFilter],
  );

  const totals = useMemo(() => {
    const accountSet = new Set(filteredRows.map((r) => r.accountId));
  ...
  const mainCurrency = filteredRows[0]?.currency ?? 'BRL';

  return (
    <>
      {untrackedSales > 0 && (
        <div className="banner">
          ⚠️ {untrackedSales} venda(s) não trackeada(s) — sem campanha correspondente identificada
          no período.
        </div>
      )}
      ...
      <div className="toolbar">
        ...
        <span className="spacer" />
        <span className="muted">{filteredRows.length} campanhas</span>
      </div>
            ...
            {!loading && filteredRows.length === 0 && (
              <tr>
                <td colSpan={12} className="empty">
                  Nenhuma campanha encontrada. Clique em "Sincronizar campanhas" para buscar da Meta.
                </td>
              </tr>
            )}
            {!loading &&
          ...
          {!loading && filteredRows.length > 0 && (
            <tfoot>
              <tr className="total-row">
                <td colSpan={2}>{filteredRows.length} CAMPANHAS</td>
